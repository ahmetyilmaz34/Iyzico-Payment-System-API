...
const randomColorGenerator = () => {
  // Profil fotografı olmadığı durumlarda kullanılacak profil rengi 
...
const UsersSchema = new Schema({
  ...
  avatarUrl: {
    // kullanıcı resimlerini belirleyen değerler
    type: String
  },
  avatarColor: {
    // kullanıcı resimlerini belirleyen değerler
  ...
  _id: true,
  // mongodb de otomatik id oluşturur.
  collection: "users",
  timestamps: true,
  toJSON: {
    transform: (doc, ret) => {
...
UsersSchema.pre("save", async function (next) {
  // kayıttan önce çalışan fonksiyondur.
  try {
...
Users.initializer = async () => {
  ...
  if (count === 0) {
    // veri yoksa örnek kullanıcıyı otomatik olarak oluşturacak.
